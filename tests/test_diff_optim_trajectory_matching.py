"""Integration-level optimization sanity check for the differentiable rigid
solver. Beyond local FD-vs-analytical agreement (which `test_diff_forward_kinematics`
covers), this test rolls a multi-step trajectory and asks: can plain Adam on
the initial `dofs_velocity` parameter recover the trajectory that the same
scene actually produced from a target init?

The test is structured as:

  1. Pick a `target_init_vel` (deterministic seed; per-env distinct if
     n_envs > 0) and roll N steps through a non-differentiable scene.
     Record the final qpos + dofs_vel as targets.
  2. Build a differentiable scene; initialize a learnable `init_vel`
     parameter at `target_init_vel + small noise`.
  3. Adam loop: rollout N steps → MSE loss on final state → backward → step.
  4. Assert (a) loss decreases by ≥2 orders of magnitude (per env) and
     (b) the final loss is below an absolute threshold — i.e. the
     diff-mode backward gives an informative descent direction over a
     multi-step horizon, not just a locally correct gradient.

Parametrized over precision ∈ {fp64, fp32} and n_envs ∈ {0 (single), 4
(batched, per-env distinct target / init)}. fp32 uses looser thresholds
to account for the lower precision floor.
"""
...
def test_diff_optim_init_vel_cartpole(show_viewer, n_envs, precision):
    ...
    if not os.environ.get("GENESIS_DIFF_OPTIM_NO_PLOT"):
        try:
            ...
            tag = f"{precision}_{'batched' if n_envs > 0 else 'single'}"
            default_out = (
...
@pytest.mark.parametrize("precision", _PRECISION_PARAMS)
@pytest.mark.parametrize("n_envs", _N_ENVS_PARAMS)
def test_diff_optim_control_force_cartpole(show_viewer, n_envs, precision):
