"""Unit tests for the `scene.backward(loss)` / `scene.reset_grad()` API and the
SHAC-style horizon-truncation pattern.
...
def test_horizon_truncation_matches_independent_scenes(mjcf_str, n_dofs, n_envs, precision_str):
    ...
    # (replacing the manual get_state / backward / reset(snapshot) dance).
