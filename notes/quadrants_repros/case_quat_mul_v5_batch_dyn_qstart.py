"""Variant 5: add batch dim + dynamic q_start (read from array)."""
...
def step1_vel_next(vel, acc, vel_next):
    for i_d, i_b in qd.ndrange(3, 1):  # batch dim
        vel_next[i_d, i_b] = vel[i_d, i_b] + DT * acc[i_d, i_b]


@qd.func
def step2_rotation(qpos, vel_next, qpos_next, q_starts):
    for i_l, i_b in qd.ndrange(1, 1):  # batch dim + dynamic loop
        q_start = q_starts[i_l]  # dynamic, from array
        rot0 = qd.Vector(
            [
                qpos[q_start + 0, i_b],
                qpos[q_start + 1, i_b],
...
def kernel_v5(
    ...
    step2_rotation(qpos, vel_next, qpos_next, q_starts)
...
def main():
    # q_start = 3 (rotation slot in a 7-dim qpos)
    Q_START = 3
    ...
    seed()
    kernel_v5(vel, acc, qpos, vel_next, qpos_next, q_starts)
    kernel_v5.grad(vel, acc, qpos, vel_next, qpos_next, q_starts)
    ...
    if float(np.abs(diff).max()) > 1e-10:
        print("===> SILENT DROP REPRODUCED in v5 <===")
    else:
        print("===> chain matches; v5 not the trigger <===")
