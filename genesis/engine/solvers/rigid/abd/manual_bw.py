...
@qd.kernel(fastcache=True)
def kernel_manual_func_integrate_bw(
    f: qd.int32,
    dofs_state: array_class.DofsState,
    links_info: array_class.LinksInfo,
    joints_info: array_class.JointsInfo,
    ...
    # Manual reverse of `func_integrate` (forward_dynamics.py line 1245-1362).
    #
    # Forward chain (per DOF):
    #   vel_next[i] = vel[i] + dt * acc[i]
    # Forward chain (per link, FREE):
    #   pos        = qpos[q_start..q_start+3]
    #   dpos       = vel_next[dof_start..dof_start+3] * dt
    #   pos_next   = pos + dpos
    #   qpos_next[0..3] = pos_next
    #   rot0       = qpos[q_start+3..q_start+7]                  (pre-integrate quat)
    #   ang        = vel_next[dof_start+3..dof_start+6] * dt
    #   qrot       = qd_rotvec_to_quat(ang, EPS)
    #   rot        = qd_transform_quat_by_quat(qrot, rot0) = qd_quat_mul(rot0, qrot)
    #   qpos_next[3..7] = rot
    # Forward chain (per DOF, REVOLUTE/PRISMATIC):
    #   qpos_next[q_start+i] = qpos[q_start+i] + vel_next[dof_start+i] * dt
    #
    # The forward primal `rot0` MUST come from the adjoint_cache (= pre-integrate
    # qpos), not from `rigid_global_info.qpos` which is post-integrate after the
    # `kernel_copy_next_to_curr_no_check` in the backward path. `vel_next` is read
    # from `dofs_state.vel_next` (forward replay output).
    #
    # Reverse outputs (additive .grad write):
    #   qpos.grad[..] from qpos_next.grad[..]
    #   vel_next.grad[..] from qpos_next.grad[..]
    #   vel.grad[..] from vel_next.grad[..]
    #   acc.grad[..] from vel_next.grad[..]
    #
    # SPHERICAL is not supported and triggers errno (per guide P9).
    ...
    for i_e, i_b in qd.ndrange(n_entities, _B):
        for i_l in range(entities_info.link_start[i_e], entities_info.link_end[i_e]):
            ...
            if joint_type == gs.JOINT_TYPE.FREE:
                ...
                # Rotation reverse — SCALAR EXPANSION of `rot = quat_mul(rot0, qrot)`
                # to avoid `qd.Vector` arithmetic. Quadrants compiles `qd.Vector`
                # ops with FMA fusion, which produces a ~1e-11 numerical diff vs
                # python numpy on near-identity quats (production state). Pure
                # scalar code with explicit left-to-right reduction order matches
                # numpy's strict IEEE 754 multiply-then-add.
                rot0_w = rigid_adjoint_cache.qpos[f, q_start + 3, i_b]
                rot0_x = rigid_adjoint_cache.qpos[f, q_start + 4, i_b]
                rot0_y = rigid_adjoint_cache.qpos[f, q_start + 5, i_b]
                rot0_z = rigid_adjoint_cache.qpos[f, q_start + 6, i_b]

                ang_x = dofs_state.vel_next[dof_start + 3, i_b] * dt
                ang_y = dofs_state.vel_next[dof_start + 4, i_b] * dt
                ang_z = dofs_state.vel_next[dof_start + 5, i_b] * dt
                ...
                # rot.grad
                rot_grad_w = rigid_global_info.qpos_next.grad[q_start + 3, i_b]
                rot_grad_x = rigid_global_info.qpos_next.grad[q_start + 4, i_b]
                rot_grad_y = rigid_global_info.qpos_next.grad[q_start + 5, i_b]
                rot_grad_z = rigid_global_info.qpos_next.grad[q_start + 6, i_b]

                # d_quat_mul__dlhs(rot0, qrot, rot_grad) — d/d(rot0_*)
                # Each product computed into a separate temp first, then summed.
                # This pattern prevents Quadrants from fusing mul+add into a
                # single FMA, which would round once (single rounding) and
                # produce a different value than numpy's strict mul-then-add
                # (two roundings). See diffrigid_handoff_n_ge_2_residual.md.
                ...
                # d_quat_mul__drhs(rot0, qrot, rot_grad) — d/d(qrot_*)
                ...
                # d_rotvec_to_quat__drotvec(ang, EPS, qrot_grad) — d/d(ang_*)
                # rotvec_to_quat: qrot_w = cos_h, qrot_{xyz} = sinc * ang_{xyz}
                # ∂qrot_w/∂ang_i = -0.5 * sin_h * ang_i / theta_reg
                # ∂(sinc·ang_j)/∂ang_i = δ_ij * sinc + ang_j * (∂sinc/∂theta_reg) * (ang_i/theta_reg)
                # ∂sinc/∂theta_reg = (0.5*cos_h - sinc) / theta_reg
                dsinc_dtheta = 0.5 * cos_h - sinc
                dsinc_dtheta = dsinc_dtheta / theta_reg

                # qg_dot_r = qrot_x_grad * ang_x + qrot_y_grad * ang_y + qrot_z_grad * ang_z
                qg_dot_r = qrot_x_grad * ang_x
                qg_dot_r = qg_dot_r + qrot_y_grad * ang_y
                qg_dot_r = qg_dot_r + qrot_z_grad * ang_z

                # coeff = -0.5 * sin_h / theta_reg * qrot_w_grad + dsinc_dtheta / theta_reg * qg_dot_r
                ...
                # Accumulate rot0.grad (= qpos.grad[3..6])
                rigid_global_info.qpos.grad[q_start + 3, i_b] = (
                    rigid_global_info.qpos.grad[q_start + 3, i_b] + rot0_w_grad
                )
                rigid_global_info.qpos.grad[q_start + 4, i_b] = (
                    rigid_global_info.qpos.grad[q_start + 4, i_b] + rot0_x_grad
                )
                rigid_global_info.qpos.grad[q_start + 5, i_b] = (
                    rigid_global_info.qpos.grad[q_start + 5, i_b] + rot0_y_grad
                )
                rigid_global_info.qpos.grad[q_start + 6, i_b] = (
                    rigid_global_info.qpos.grad[q_start + 6, i_b] + rot0_z_grad
                )

                # Accumulate vel_next.grad[3..5]
                dofs_state.vel_next.grad[dof_start + 3, i_b] = (
                    dofs_state.vel_next.grad[dof_start + 3, i_b] + dt * ang_grad_x
                )
                dofs_state.vel_next.grad[dof_start + 4, i_b] = (
                    dofs_state.vel_next.grad[dof_start + 4, i_b] + dt * ang_grad_y
                )
                dofs_state.vel_next.grad[dof_start + 5, i_b] = (
                    dofs_state.vel_next.grad[dof_start + 5, i_b] + dt * ang_grad_z
                )
                ...
                for i_d_ in range(links_info.n_dofs[I_l]):
                    qn_g = rigid_global_info.qpos_next.grad[q_start + i_d_, i_b]
                    rigid_global_info.qpos.grad[q_start + i_d_, i_b] = (
                        rigid_global_info.qpos.grad[q_start + i_d_, i_b] + qn_g
                    )
                    dofs_state.vel_next.grad[dof_start + i_d_, i_b] = (
                        dofs_state.vel_next.grad[dof_start + i_d_, i_b] + dt * qn_g
                    )

    # vel.grad / acc.grad from vel_next = vel + dt * acc
    qd.loop_config(serialize=static_rigid_sim_config.para_level < gs.PARA_LEVEL.ALL)
