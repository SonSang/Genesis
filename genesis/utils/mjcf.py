...
def parse_geom(mj, i_g, scale, surface, xml_path):
    ...
    info = {
        ...
        # In MuJoCo, friction[1] (torsional) is only honored when condim >= 4. With the default
        # condim=3, slide-only friction is used. We mirror this convention so existing MJCF assets
        # that rely on the default condim do not silently change behavior.
